...
crir = {
		...
		if (! this.isSafari) { // the script doesn't work in safari.
			...
			for (var i=0; i<arrLabels.length; i++) {			
				...
				if (inputElementClass == 'crirHiddenJS') {
					...
					if (inputElement.getAttributeNode('type') != null) {
						inputElementType = inputElement.getAttributeNode('type').value;	

						// keep the exisiting onclick event if one exists
						var onclickCurrent = inputElement.onclick;
						inputElement.onclick = null;
						if (onclickCurrent) {
							crir.addEvent(inputElement, 'click', onclickCurrent, false);
						}
				...
				else if (inputElement.nodeName != 'SELECT' && inputElement.getAttributeNode('type') != null && inputElement.getAttributeNode('type').value == 'radio') { // this so even if a radio is not hidden but belongs to a group of hidden radios it will still work.
	...
	toggleRadioLabel: function () {	
		...
		for (var i=0; i<arrInputs.length; i++) {
			if (arrInputs[i].getAttributeNode('type') != null) {
