    ...
    <div class="container">
      ...
      <div class="hero-unit">
		...
		<struts-html:form styleId="loginform" styleClass="form-signin" action="/client/scheduleFilter" focus="password">
			  ...
			  <h3 class="form-signin-heading">What kind of appointment?</h3>
<%
Iterator types_itr = appt_types.iterator();
while (types_itr.hasNext()) {
	AppointmentTypeBean type = (AppointmentTypeBean)types_itr.next();
	if (type.getType() == AppointmentTypeBean.CLIENT_APPOINTMENT_TYPE_TYPE) {
%>
			  <button style="width: 100%; margin-top: 5px;" type="submit" name="appointment_type_id" value="<%= type.getValue() %>" class="btn btn-large<%= (type.getId() == selected_appointment_type.getId()) ? " btn-primary" : ""%>"><%= type.getLabel() %></button>
<%
	}
}
%>
			  ...
			  <p>&nbsp</p>
			  <a style="margin-left: 10px;" class="btn btn-large" href="schedule.jsp">Go back</a>
