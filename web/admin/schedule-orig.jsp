...
try
{
    System.out.println("practice_area_id >" + request.getParameter("practice_area_id"));
    System.out.println("adminPracticeArea.isNew() >" + adminPracticeArea.isNew());
	...
	else
	{
	    ...
	    System.out.println("adminPracticeArea1 >" + adminPracticeArea.getLabel());
	    practitioners = adminPracticeArea.getPractitioners();
	}
	System.out.println("adminPracticeArea2 >" + adminPracticeArea.getLabel());
	session.setAttribute("adminPracticeArea", adminPracticeArea);
	System.out.println("adminPracticeArea3 >" + adminPracticeArea.getLabel());
    }
    else
    {
...
System.out.println("adminPracticeArea4 >" + adminPracticeArea.getLabel());
...
%>
	...
	//alert(this.value);
	document.location.href='schedule-orig.jsp?practice_area_id=' + this.value;
...
<div id="headline">
	...
	<div id="explore">
		<h2>Valeo</h2>
		<p><strong><span id="today_date"><%= date_format.format(display_date.getTime())%></span></strong></p>
		...
		<select id="menubutton2select" name="menubutton2select">
		    <option value="0" onclick="alert('gramp');">All</option>
		    <option value="1" onclick="alert('cramp');">Chiropractic</option>
        ...
        <div id="demo" class="yui-navset">
            <ul class="yui-nav">
                ...
                <li><a href="#tab2"><em>No Show</em></a></li>
            </ul>            
            <div class="yui-content">
                <div id="tab1">
                    <p>Tab One Content</p>
                    <select name="checkedInSelect" id="checkedInSelect" size="9" style="width:100%;" onclick="showClientInfo(this.value);">
                    </select>
                </div>
                <div id="tab2">
                    <p>Tab Two Content</p>
                    <select name="lateSelect" id="lateSelect" size="9" style="width:100%;" onclick="showClientInfo(this.value);">
                    </select>
                </div>
                <div id="tab3">
                    <p>Tab Three Content</p>
                    <select name="noShowSelect" id="noShowSelect" size="9" style="width:100%;" onclick="showClientInfo(this.value);">
                    </select>
                </div>
            </div>
...
%>
                <td onclick="cClk('<%= practitioner.getId() %>','<%= practitioner.getLabel() %>','<%= time_str %>');" onmouseover="cOvr('<%= time_str + "|" + practitioner.getId() %>');" id="<%= time_str + "|" + practitioner.getId() %>">&nbsp;</td>
<%
    }
%>
            </tr>
...
<div id="dialog1">
	...
	<form name="newApptForm" method="POST" action="<%= CUBean.getProperty("cu.formSubmitPath") %>/newAppointment.x"  >
	    ...
	    <label for="practitioner"><strong>Practitioner:</strong></label>
	    <span id="practitioner">&nbsp;</span>
	    <input type="hidden" name="practitionerSelect" value="">

	    <div class="clear"></div>

	    <label for="new_appt_date"><strong>Date:</strong></label>
	    <span id="new_appt_date"><%= date_format.format(display_date.getTime()) %></span>
	    <input type="hidden" name="newAppointmentDateInput" value="<%= date_format.format(display_date.getTime()) %>">
	    ...
	    <label for="lastname"><strong>Search Last Name:</strong></label><input id="lastname" type="textbox" onkeyup="if (document.getElementById('lastname').value.length > 0) {processCommand('getPeopleByLastName', document.getElementById('lastname').value);}" name="lastname" />
	    <label for="firstname"><strong>Search First Name:</strong></label><input type="textbox" name="firstname" />

	    <div class="clear"></div>

	    <label for="clientSelect"><strong>Client:</strong></label>
	    <select multiple name="clientSelect">
...
<%
type_itr = AppointmentTypeBean.getAppointmentTypes(adminCompany, AppointmentTypeBean.CLIENT_APPOINTMENT_TYPE_TYPE).iterator();
while (type_itr.hasNext())
{
    AppointmentTypeBean appointmentType = (AppointmentTypeBean)type_itr.next();
%>
...
%>
	    ...
	    <label for="duration"><strong>Duration:</strong></label><input type="textbox" name="duration" />
