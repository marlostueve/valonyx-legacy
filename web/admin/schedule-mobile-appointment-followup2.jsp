...
if (request.getParameter("type") != null) {
	AppointmentTypeBean selectedType = AppointmentTypeBean.getAppointmentType(Integer.parseInt(request.getParameter("type")));
	followupAppointment.setType(selectedType);
	followupAppointment.setDuration(selectedType.getDuration());
}
	...
	<div id="wrap">
		...
		<div id="main">
			...
			<div class="header">
				...
				<a class="left" href="schedule-mobile-appointment-followup.jsp">Back</a>
			...
			<div class="content">
				...
				<h2 class="title">Select Followup Appointment Date</h2>
				
				<!-- <p><%= longer_date_format.format(fuCalendar.getTime()) %></p> -->
				

<%
fuCalendar.set(Calendar.DAY_OF_WEEK, Calendar.SUNDAY);
%>

				<p>
					<a class="likeabutton" href="schedule-mobile-appointment-followup2.jsp?display=previous">previous</a>
				</p>
				<p>
<%
for (int i = 0; i < 7; i++) {
	...
	if (fuCalendar.get(Calendar.DATE) == selectedCalendar.get(Calendar.DATE)) {
		if (fuCalendar.get(Calendar.MONTH) == selectedCalendar.get(Calendar.MONTH)) {
			if (fuCalendar.get(Calendar.YEAR) == selectedCalendar.get(Calendar.YEAR)) {
				equals = true;
			}
		}	
	}
	
	int display_year = fuCalendar.get(Calendar.YEAR);
%>
					<a class="<%= equals ? "buttoncurrent" : (i == 0 || i == 6) ? "likeabuttonwe" : "likeabutton" %>" href="schedule-mobile-appointment-followup3.jsp?display=<%= fuCalendar.getTimeInMillis() %>"><%= ((fuCalendar.get(Calendar.DATE) < 10) ? "0" : "") + fuCalendar.get(Calendar.DATE) %></a>
<%
	
	fuCalendar.add(Calendar.DATE, 1);
}
//fuCalendar.add(Calendar.DATE, -7);
%>
				</p>
...
fuCalendar.add(Calendar.DATE, -28);
%>
				</p>
				<p>
					<a class="likeabutton" href="schedule-mobile-appointment-followup2.jsp?display=next">next</a>
				</p>
