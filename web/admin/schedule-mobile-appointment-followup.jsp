...
followupAppointment = new AppointmentBean();
session.setAttribute("followupAppointment", followupAppointment);

followupAppointment.setClient(appointment.getClient());


if (request.getParameter("id") != null) {
	UKOnlinePersonBean fuPrac = (UKOnlinePersonBean)UKOnlinePersonBean.getPerson(Integer.parseInt(request.getParameter("id")));
	followupAppointment.setPractitioner(fuPrac);
}
	...
	<div id="wrap">
		...
		<div id="main">
			...
			<div class="header">
				...
				<a class="left" href="schedule-mobile-appointment.jsp">Back</a>
			...
			<div class="content">
				...
				<h2 class="title">Select an Appointment Type</h2>
<%
HashMap practice_areas_hash = new HashMap();
java.util.Iterator type_itr = com.badiyan.uk.online.beans.AppointmentTypeBean.getAppointmentTypes(company).iterator();
while (type_itr.hasNext()) {
    ...
    if (appointmentType.getPracticeAreaId() > 0) {
		...
		Vector appointment_types_in_practice_area = (Vector)practice_areas_hash.get(appointmentType.getPracticeArea().getLabel());
		if (appointment_types_in_practice_area == null) {
			appointment_types_in_practice_area = new Vector();
			System.out.println("putting >" + appointmentType.getPracticeArea().getLabel());
			practice_areas_hash.put(appointmentType.getPracticeArea().getLabel(), appointment_types_in_practice_area);
		}
		System.out.println("add appt type >" + appointmentType.getLabel());
		appointment_types_in_practice_area.addElement(appointmentType);
	}
}
...
%>
				...
				<p><%= practice_area.getLabel() %></p>

				<div class="box-white">
					
<%
	Vector appointment_types_in_practice_area = (Vector)practice_areas_hash.get(practice_area.getLabel());
	System.out.println("appointment_types_in_practice_area >"  + appointment_types_in_practice_area);
	if (appointment_types_in_practice_area != null) {
		Iterator app_type_itr = appointment_types_in_practice_area.iterator();
		while (app_type_itr.hasNext()) {
			AppointmentTypeBean appointmentType = (AppointmentTypeBean)app_type_itr.next();
			if (appointmentType.getType() == AppointmentTypeBean.CLIENT_APPOINTMENT_TYPE_TYPE) {
%>
					<p><a href="schedule-mobile-appointment-followup2.jsp?type=<%= appointmentType.getId() %>"><%= appointmentType.getLabel() %><span><span class="arrow">&gt;</span></span></a></p>
