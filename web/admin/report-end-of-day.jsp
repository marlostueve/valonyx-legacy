...
BigDecimal total_tax = BigDecimal.ZERO;
int num_orders = 0;
...
HashMap order_hash = new HashMap();
HashMap order_person_hash = new HashMap();
//HashMap order_hash = new HashMap();

Vector orders_x = new Vector();
Vector orders_y = new Vector();
...
String report_label = "End of Day Financial Report";
...
try
{
    ...
    while (obj_itr.hasNext())
    {
		...
		ValeoOrderBean order_obj = (ValeoOrderBean)ValeoOrderBean.getOrder(obj.getOrderid());
		if (!orders_y.contains(order_obj))
			orders_y.addElement(order_obj);
		
		Vector orderlines = (Vector)order_hash.get(order_obj);
		if (orderlines == null) {
			orderlines = new Vector();
			order_hash.put(order_obj, orderlines);
		}
		orderlines.addElement(obj);
		
		try	{
			UKOnlinePersonBean client_for_order = order_obj.getClient();
			order_person_hash.put(order_obj, client_for_order);
		}
		catch (Exception x) {
			x.printStackTrace();
		}
    ...
    if (courseReportLister.showExcel())
    {
		...
		courseReportLister.addCell(sheet, 4, (short)0, "Financial Report", total_style);
		...
		if (orders_x.size() > 0)
		{
			...
			while (itr.hasNext())
			{
				...
				if (last_department_str != null && !department_str.equals(last_department_str))
				{
					...
					courseReportLister.addCell(sheet, row_index, column_index, "", style); column_index++;
					courseReportLister.addCell(sheet, row_index, column_index, "", style); column_index++;
					courseReportLister.addCell(sheet, row_index, column_index, "stuff", right_style); column_index++;
					*/
...
<div id="top1">
    ...
    <div id="center1" style="background-color: white;">
		...
		<div class="content_TextBlock">
			<p class="headline"><%= adminCompany.getLabel() %> Administration - <%= report_label %></p>
					...
					<div class="heading">
						<table cellspacing="0" cellpadding="0" border="0" summary="">
							<tr style="display: block;">
								<td style="width: 366px; text-align: left;  "><a href="report-end-of-day.jsp?sort=<%= (courseReportLister.getSort() == (short)1) ? 2 : 1  %>" title="">Description</a></td>
								<td style="width:  66px; text-align: right;  "><a href="report-end-of-day.jsp?sort=<%= (courseReportLister.getSort() == (short)3) ? 4 : 3  %>" title="">Qty</a></td>
								<td style="width:  66px; text-align: right;  "><a href="report-end-of-day.jsp?sort=<%= (courseReportLister.getSort() == (short)5) ? 6 : 5  %>" title="">Price</a></td>
								<td style="width:  66px; text-align: right;  "><a href="report-end-of-day.jsp?sort=<%= (courseReportLister.getSort() == (short)7) ? 8 : 7  %>" title="">Tax</a></td>
								<td style="width:  66px; text-align: right;  "><a href="report-end-of-day.jsp?sort=<%= (courseReportLister.getSort() == (short)9) ? 10 : 9  %>" title="">Total</a></td>
...
<%
			...
			Iterator itr = orders_y.iterator();
			//Iterator itr = codes.iterator();
			while (itr.hasNext())
			{
				//CheckoutCodeBean checkout_code = (CheckoutCodeBean)itr.next();
				ValeoOrderBean order_obj = (ValeoOrderBean)itr.next();
				UKOnlinePersonBean client = order_obj.getClient();
				num_orders++;
				
				BigDecimal subtotal_tax = BigDecimal.ZERO;
				BigDecimal subtotal_total = BigDecimal.ZERO;
%>

						<table cellspacing="0" cellpadding="0" border="0" summary="">
							<tr>
								<td style="width: 366px; text-align: left; vertical-align: top;  ">
									<%= CUBean.getUserDateString(order_obj.getOrderDate()) %> <%= CUBean.getUserTimeString(order_obj.getOrderDate()) %> - 
									<a href="schedule-administration-checkout-code.jsp?id=<%= order_obj.getValue() %>"><%= order_obj.getLabel() %></a> - 
									<a href="clients-new.jsp?id=<%= client.getValue() %>"><%= client.getLabel() %></a>
								</td>
								<td style="width:  66px; text-align: right; vertical-align: top;  ">&nbsp;</td>
								<td style="width:  66px; text-align: right; vertical-align: top; ">&nbsp;</td>
								<td style="width:  66px; text-align: right; vertical-align: top; ">&nbsp;</td>
								<td style="width:  66px; text-align: right; vertical-align: top; ">&nbsp;</td>
...
<%
				...
				while (orderline_itr.hasNext())
				{
					...
					*/
					...
					BigDecimal qty = orderline_obj.getQuantity();
					BigDecimal price = orderline_obj.getPrice();
					BigDecimal tax = orderline_obj.getTax();
					if (tax == null)
						tax = BigDecimal.ZERO;
					BigDecimal total = orderline_obj.getActualAmount();
					if (total == null)
					...
					subtotal_tax = subtotal_tax.add(tax);
					subtotal_total = subtotal_total.add(total);
					
%>

							<tr>
								<td style="width: 366px; text-align: left; vertical-align: top;  "><a href="schedule-administration-checkout-code.jsp?id=<%= checkout_code.getValue() %>"><%= checkout_code.getLabel() %></a></td>
								<td style="width:  66px; text-align: right; vertical-align: top;  "><%= qty.setScale(2, BigDecimal.ROUND_HALF_UP).toString() %></td>
								<td style="width:  66px; text-align: right; vertical-align: top; "><%= price.setScale(2, BigDecimal.ROUND_HALF_UP).toString() %></td>
								<td style="width:  66px; text-align: right; vertical-align: top; "><%= tax.setScale(2, BigDecimal.ROUND_HALF_UP).toString() %></td>
...
%>
						...
						<table cellspacing="0" cellpadding="0" border="0" summary="">
							...
							<tr>
								<td style="width: 366px; text-align: left; vertical-align: top;  ">&nbsp;</td>
								<td style="width:  66px; text-align: left; vertical-align: top;  ">&nbsp;</td>
								<td style="width:  66px; text-align: right; vertical-align: bottom; ">&nbsp;</td>
								<td style="width:  66px; text-align: right; vertical-align: bottom; "><strong><%= subtotal_tax.setScale(2, BigDecimal.ROUND_HALF_UP).toString() %></strong></td>
								<td style="width:  66px; text-align: right; vertical-align: bottom; "><strong><%= subtotal_total.setScale(2, BigDecimal.ROUND_HALF_UP).toString() %></strong></td>
								<td style="width:  68px; text-align: left;  ">&nbsp;</td>
							</tr>
...
%>
						...
						<table cellspacing="0" cellpadding="0" border="0" summary="">
							...
							<tr>
								<td style="width: 366px; text-align: left; vertical-align: top;  "><strong>TOTAL (<%= num_orders %> Order<%= num_orders == 1 ? "" : "s" %>)</strong></td>
								<td style="width:  66px; text-align: left; vertical-align: top;  ">&nbsp;</td>
								<td style="width:  66px; text-align: right; vertical-align: bottom; ">&nbsp;</td>
								<td style="width:  66px; text-align: right; vertical-align: bottom; "><strong><%= total_tax.setScale(2, BigDecimal.ROUND_HALF_UP).toString() %></strong></td>
								<td style="width:  66px; text-align: right; vertical-align: bottom; "><strong><%= total_total.setScale(2, BigDecimal.ROUND_HALF_UP).toString() %></strong></td>
								<td style="width:  68px; text-align: left;  ">&nbsp;</td>
							</tr>
