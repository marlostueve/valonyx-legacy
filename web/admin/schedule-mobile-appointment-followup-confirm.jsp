 ...
 */
	...
	fuCalendar.set(Calendar.MINUTE, minute);
	fuCalendar.set(Calendar.SECOND, 0);
	
	
	
	followupAppointment.setAppointmentDate(fuCalendar.getTime());
...
if (request.getParameter("save") != null) {
	...
	followupAppointment.setCompany(adminCompany);
...
color: #fff;border-radius: 5px;border: 1px solid #6d6d6d;font-size: 12px;left: 310px;position: fixed;top: 9px;padding: 5px 8px;text-decoration:none;}
		...
		#main .content {}
			...
			#main table {margin:20px 0 10px;width:100%;}
				...
				#main .content .button.blue {background: #0085d5;}
	...
	<div id="wrap">
		...
		<div id="main">
			...
			<div class="header">
				...
				<a class="left" href="schedule-mobile-appointment-followup4.jsp">Back</a>
...
<%
if (followupAppointment.isNew()) {
%>
				<p>Click <strong>Schedule Appointment</strong> to save this follow-up appointment.</p>
<%
} else {
%>
				<p>Follow-up appointment saved.</p>
<%
}
%>
...
Date appt_date = followupAppointment.getAppointmentDate();

Calendar appt_end = Calendar.getInstance();
appt_end.setTime(appt_date);
appt_end.add(Calendar.MINUTE, followupAppointment.getDuration());

String status_string = "";
if (followupAppointment.isCancelled())
	status_string = "[CANCELLED] ";
else if (followupAppointment.isRescheduled())
	status_string = "[RESCHEDULED] ";

String comments = followupAppointment.getComments();
if (!comments.equals("") || !status_string.equals(""))
	comments = status_string + comments;


%>
					<p>Client <span><a href="?"><strong><%= followupAppointment.getClient().getLabel() %></strong></a></span></p>
					...
					<p>Type <span><%= followupAppointment.getType().getLabel() %></span></p>
					<p>Practitioner <span><%= followupAppointment.getPractitioner().getLabel() %></span></p>
...
if (followupAppointment.isNew()) {
%>
				<p><input type="submit" class="button red" value="Schedule Appointment" onclick="document.location.href='schedule-mobile-appointment-followup-confirm.jsp?save=true'" /></p>
<%
} else {
%>
				<p><input type="submit" class="button blue" value="Finish" onclick="document.location.href='schedule-mobile-appointment.jsp'" /></p>
<%
}
%>
