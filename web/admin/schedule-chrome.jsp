...
Vector practitioners = PracticeAreaBean.getAllPractitioners();
/*
try
{
    ...
    if (request.getParameter("practice_area_id") != null)
    {
		if (request.getParameter("practice_area_id").equals("0")) {
			...
			vec = adminPracticeArea.getPractitioners();
		}
	...
	if (vec != null) {
		...
		Iterator prac_itr = vec.iterator();
		while (prac_itr.hasNext()) {
			UKOnlinePersonBean practitioner = (UKOnlinePersonBean)prac_itr.next();
...
UKOnlinePersonBean bev_obj = (UKOnlinePersonBean)practitioners.remove(11);
practitioners.insertElementAt(bev_obj, 7);

UKOnlinePersonBean lesya_obj = (UKOnlinePersonBean)practitioners.remove(0);
practitioners.insertElementAt(lesya_obj, 4);
} catch (Exception x) {
...
while (practitioner_itr.hasNext())
{
    ...
    System.out.println("FOUND PRAC >" + practitioner.getLabel());
%>
	...
	if (httpRequest.readyState == 4)
	{
	    if (httpRequest.status == 200)
	    {
			...
			else if (httpRequest.responseXML.getElementsByTagName("codes").length > 0)
			{
				...
				showCodes(xml_response, xml_response.getAttribute("add"));
                                /*
                                if (xml_response.getAttribute("add")) {
...
for (int i = 2; practitioner_itr.hasNext(); i++)
{
	...
	} else if (room_label.equals("TX 2")) {
		room_label = "Lisa - TX2";
	} else if (room_label.equals("Kevin")) {
		room_label = "Kevin - BW1";
	...
	} else if (room_label.equals("Michelle")) {
		room_label = "Leah - TX3";
	} else if (room_label.equals("Stori")) {
		room_label = "Stori - Lab/Therm";
	}
	
%>
                <th id="h<%= i %>"><a href="practitioners-chrome.jsp?id=<%= practitioner.getId() %>"><strong><%= room_label %></strong></a></th>
<%
}
%>
            </tr>
</thead>
<tbody id="sc_cont" class="scrollContent" style="height: 100%;">
