...
UKOnlineCompanyBean company = (UKOnlineCompanyBean)UKOnlineCompanyBean.getCompany(5); // this needs to be fixed
	...
	<div id="wrap">
		...
		<div id="main">
			...
			<div class="header">
				...
				<!-- <a class="left" href="?">&nbsp;</a> -->
				<h1 class="title">Practitioner Schedules</h1>
			...
			<div class="content">
				...
				<h2 class="title">Practitioners</h2>
...
//UKOnlinePersonBean selected_practitioner = null;
if (request.getParameter("id") != null) {
	selected_practitioner = (UKOnlinePersonBean)UKOnlinePersonBean.getPerson(Integer.parseInt(request.getParameter("id")));
...
%>
					<p><a href="schedule-mobile.jsp?id=<%= practitioner.getValue() %>"><%= practitioner.getLabel() %><span><span class="arrow">&gt;</span></span></a></p>
<%
	if (selected_practitioner == null) {
		selected_practitioner = practitioner;
		session.setAttribute("selected_practitioner", selected_practitioner);
	} else if (selected_practitioner.isNew()) {
		selected_practitioner = practitioner;
		session.setAttribute("selected_practitioner", selected_practitioner);
	}
}
...
Calendar displayCalendar = (Calendar)session.getAttribute("displayCalendar");
if (displayCalendar == null) {
	displayCalendar = Calendar.getInstance();
	session.setAttribute("displayCalendar", displayCalendar);
}

Calendar selectedCalendar = (Calendar)session.getAttribute("selectedCalendar");
if (selectedCalendar == null) {
	selectedCalendar = Calendar.getInstance();
	session.setAttribute("selectedCalendar", selectedCalendar);
}


if (request.getParameter("display") != null) {
	if (request.getParameter("display").equals("previous")) {
		displayCalendar.add(Calendar.DATE, -7);
	} else if (request.getParameter("display").equals("next")) {
		displayCalendar.add(Calendar.DATE, 7);
	} else {
		displayCalendar.setTimeInMillis(Long.parseLong(request.getParameter("display")));
...
%>
				...
				<h2 class="title"><%= selected_practitioner.getLabel() %> Schedule for <%= short_date_format.format(selectedCalendar.getTime()) %></h2>
				

<%
displayCalendar.set(Calendar.DAY_OF_WEEK, Calendar.SUNDAY);
%>

				<p>
					<a class="likeabutton" href="schedule-mobile.jsp?display=previous">previous</a>

<%
for (int i = 0; i < 7; i++) {
	
	boolean equals = false;
	if (displayCalendar.get(Calendar.DATE) == selectedCalendar.get(Calendar.DATE)) {
		if (displayCalendar.get(Calendar.MONTH) == selectedCalendar.get(Calendar.MONTH)) {
	...
	int display_year = displayCalendar.get(Calendar.YEAR);
%>
					<a class="<%= equals ? "buttoncurrent" : (i == 0 || i == 6) ? "likeabuttonwe" : "likeabutton" %>" href="schedule-mobile.jsp?display=<%= displayCalendar.getTimeInMillis() %>"><%= displayCalendar.get(Calendar.DATE) %></a>
<%
...
displayCalendar.add(Calendar.DATE, -7);
%>
					<a class="likeabutton" href="schedule-mobile.jsp?display=next">next</a>
				</p>
...
UKOnlinePersonBean practitioner = selected_practitioner;
Vector appointments = AppointmentBean.getAppointmentsForPractitioner(practitioner, selectedCalendar.getTime());
...
%>
				...
				<p><input type="submit" class="button red" value="This is a button" /></p>
				-->
...
Iterator appointments_for_selected_date_itr = AppointmentBean.getAppointmentsForDate(company, selectedCalendar.getTime()).iterator();
while (appointments_for_selected_date_itr.hasNext()) {
	AppointmentBean appointmentForDate = (AppointmentBean)appointments_for_selected_date_itr.next();
	num_total++;
	if (appointmentForDate.isCheckedIn())
		num_checked_in++;
	else if (appointmentForDate.isCheckedOut())
		num_checked_out++;
}
...
%>
				<ul class="nav">
					<li><a href="schedule-mobile.jsp" class="active"><span class="ico msg"></span>Schedule</a></li>
					<li><a href="?"><span class="ico msg"></span>Checked In<span class="info">&nbsp;<%= num_checked_in %>&nbsp;</span></a></li>
					<li><a href="?"><span class="ico msg"></span>Checked Out<span class="info">&nbsp;<%= num_checked_out %>&nbsp;</span></a></li>
					<li><a href="?"><span class="ico msg"></span>Clients<span><%= company.getPeople().size() %></span></a></li>
