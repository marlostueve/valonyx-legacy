...
<jsp:useBean id="adminRoom" class="com.badiyan.uk.online.beans.TreatmentRoomBean" scope="session" />


<%

session.setAttribute("setup_01a", "true");
...
if ((request.getParameter("id") != null))
{
    try
    {
		adminRoom = UKOnlinePersonBean.getPerson(adminCompany, Integer.parseInt(request.getParameter("id")));
    ...
    {
		x.printStackTrace();
		adminRoom = new TreatmentRoomBean();
		session.setAttribute("adminRoom", adminRoom);
    }
}


boolean found_rooms = false;

UKOnlinePersonBean.invalidatePractitioners(adminCompany);

String prac_message = "Setup the Rooms for " + adminCompany.getLabel() + ".";
Vector rooms = TreatmentRoomBean.getTreatmentRooms(adminCompany);
if (rooms.size() == 1)
{
