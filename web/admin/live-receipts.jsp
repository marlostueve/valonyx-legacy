...
<head>
    ...
    */
	...
	function refresh()
	{
		processCommand('getLastNReceipts');
		mTimer = setTimeout('refresh();',10000);
	}
    ...
    function processCommandEvent()
	{
		if (httpRequest.readyState == 4)
		{
			if (httpRequest.status == 200)
			{
				...
				else if (httpRequest.responseXML.getElementsByTagName("receipts").length > 0)
				{
					var xml_response = httpRequest.responseXML.getElementsByTagName("receipts")[0];
					showReceipts(xml_response);
				}
	...
	function showReceipts(xml_str)
	{
		removeReceiptContainer();
		
		
		var parent_container_div = document.getElementById('receiptParent');
		
		var receipt_container_div = document.createElement('div');
		receipt_container_div.setAttribute('id', 'receiptNav');
		receipt_container_div.setAttribute('class', 'subnav');
		receipt_container_div.setAttribute('style', 'font-size: larger;');
		parent_container_div.appendChild(receipt_container_div);
		
		var ul_element = document.createElement('ul');
		ul_element.setAttribute('id', 'ulElement');
		receipt_container_div.appendChild(ul_element);
		
		
		//alert('showReceipts');
		var index = 0;
		if (xml_str.getElementsByTagName("receipt")[index] != null)
		{
			
			while (xml_str.getElementsByTagName("receipt")[index] != null)
			{
				key = xml_str.getElementsByTagName("receipt")[index].childNodes[0].childNodes[0].nodeValue;
				value = xml_str.getElementsByTagName("receipt")[index].childNodes[1].childNodes[0].nodeValue;
				//alert(value);
				
				
				eval("var receiptArray = " + value);
				var receiptLabel = receiptArray["date"] + ' - ' + receiptArray["client"];
				if (receiptArray["isToday"] == 'true')
					receiptLabel = receiptArray["time"] + ' - ' + receiptArray["client"];
				showReceiptItem(key, receiptLabel, receiptArray["isSelected"]);
				
				if (receiptArray["isSelected"] == 'true') {
					
					if (lastUrl != receiptArray["url"]) {
						
						//document.getElementById('cos_div').innerHTML = '<EMBED SRC="' + receiptArray["url"] + '" WIDTH="100%" HEIGHT="100%"></EMBED>';
						document.getElementById('cos_div').innerHTML = '<iframe SRC="' + receiptArray["url"] + '" WIDTH="100%" HEIGHT="100%"></iframe>';
						panel.show();
						lastUrl = receiptArray["url"];
					}
				}
	...
	function removeReceiptContainer() {
		var parent = document.getElementById('receiptParent');
		if (parent) {
			var child = document.getElementById('receiptNav');
			if (child)
	...
	function selectReceipt(id) {
		processCommand('selectReceipt', id);
	}
	
	function showReceiptItem(id, label, sel) {
		
		var ulEleme = document.getElementById('ulElement');
		
		// <a href="clients-new.jsp" class="lvl1" title="Christine Stueve"><strong style=\"font-weight: bolder;\">07:08 pm - Christine Stueve</strong></a>
		
		/*
		 *<ul>
					<li style="font-size: larger; font-weight: bolder; "><a href="">stuff 1</a></li>
					<li>stuff 1</li>
					<li>stuff 1</li>
				</ul>
		 */
		...
		if (sel == 'true')
			li_element.setAttribute('style', 'font-size: larger; font-weight: bolder; ');
		else
			li_element.setAttribute('style', 'font-size: larger; ');
		ulEleme.appendChild(li_element);
		
		var receipt_anchor = document.createElement('a');
		receipt_anchor.setAttribute('href', 'javascript:selectReceipt(' + id + ');');
		receipt_anchor.setAttribute('class', 'lvl1');
		receipt_anchor.setAttribute('title', label + ' Receipt');
		var receipt_label = document.createTextNode(label);
		receipt_anchor.appendChild(receipt_label);
		//var receipt_br = document.createElement('br');
		li_element.appendChild(receipt_anchor);
		//receipt_nav_div.appendChild(receipt_br);
...
<body class="yui-skin-sam" onload="javascript:refresh();"  style="background-color: white;" >
...
<div id="center1" style="background-color: white;">
	...
	<div class="content_TextBlock">
	    <!-- <p class="headline"><%= adminCompany.getLabel() %> - Receipts</p> -->
	...
	<div class="content_Administration">
		...
		<div id="receiptParent">
			
			<div id="receiptNav" class="subnav">
		    ...
		    <div class="content_AdministrationTable" >
			    ...
			    <div class="ledger">
				    <table id="ledger-table" cellspacing="" cellpadding="" border="0" summary="" width="100%">
					    <tr>
						    <td style="text-align: left;  ">Click an item on the left to view receipt</td>
					    </tr>
				    </table>
			    </div>
