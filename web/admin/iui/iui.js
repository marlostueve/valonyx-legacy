...
function chapters(book,bkname) {
	document.getElementById('chapters').innerHTML = '<h3>Loading Chapters...</h3>';

	if (recReq.readyState == 4 || recReq.readyState == 0) {
		recReq.open("GET", '/newcomps/chapters.html?book_id=' + book, true);
		recReq.onreadystatechange = chapReplace;
		document.getElementById('pageTitle').innerHTML = bkname;
		recReq.send(null);
	}
}
...
function chapReplace() {
	if (recReq.readyState == 4) {
		document.getElementById('chapters').innerHTML = recReq.responseText;
	}
}

function verses(book, bkname, chapter) {
	document.getElementById('verses').innerHTML = '<h3>Loading Verses...</h3>';

	if (recReq.readyState == 4 || recReq.readyState == 0) {
		recReq.open("GET", '/newcomps/verses.html?book_id=' + book + '&chapter_id=' + chapter, true);
		recReq.onreadystatechange = verseReplace;
		document.getElementById('pageTitle').innerHTML = bkname + ' ' + chapter;
		recReq.send(null);
	}
}
...
function verseReplace() {
	if (recReq.readyState == 4) {
		document.getElementById('verses').innerHTML = recReq.responseText;
	}
}