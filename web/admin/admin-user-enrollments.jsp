...
<%@ include file="channels\channel-admin-menu.jsp" %>
					...
					<div class="main">
						...
						<p class="headlineA">User Profile&nbsp;&#47;&nbsp;Enrollments</p>

						<p class="currentObjectName"><%= adminPerson.getLabel() %></p>
						<p>Use this screen to manage enrollments for a particular user.</p>
...
<%
	...
	Date released_date_course = (Date)group_released_dates.get(curriculum_group);
	if (groups_to_display.contains(curriculum_group))
	{
...
%>
										<td style="width: 242px; text-align: left;  "><strong><%= curriculum_group.getName().toUpperCase() %></strong></td>
...
%>
												...
												<label for="displayCourseAssign_<%= lesson_group.getId() %>" style="margin-left: -3px;"><strong><%= lesson_group.getName() %></strong></label>
												<input name="assignCourse_<%= lesson_group.getId() %>" id="displayCourseAssign_<%= lesson_group.getId() %>" type="checkbox" class="crirHiddenJS"<%= can_modify ? "" : " disabled=\"true\"" %> onclick="groupClick<%= lesson_group.getId() %>();" />
												</td>
...
%>
										...
										//<![CDATA[
											...
											function
											groupClick<%= lesson_group.getId() %>()
											{
<%
			lesson_courses_itr = lesson_courses.iterator();
			while (lesson_courses_itr.hasNext())
			{
			    CourseBean course = (CourseBean)lesson_courses_itr.next();
%>
												if (document.getElementById('displayCourseAssign_<%= lesson_group.getId() %>').checked == true)
												{
												    document.getElementById('displayAssign_<%= course.getId() %>').checked = "checked";
												    crir.findLabel('displayAssign_<%= course.getId() %>').className = "checkbox_checked";
												}
												else
												{
												    document.getElementById('displayAssign_<%= course.getId() %>').checked = "";
												    crir.findLabel('displayAssign_<%= course.getId() %>').className = "checkbox_unchecked";
												}
<%
			}
%>
...
%>
							...
							<div class="certification">
								<table cellspacing="0" cellpadding="0" border="0" summary="">
									<tr>
										<td style="width: 492px; text-align: left;  " colspan="4">No Enrollments Found</td>
									</tr>
								</table>
							</div>
							<!-- END Course -->
...
%>
							<div class="adminItem">
								You don't have permission to modify enrollments.
							</div>
<%
}
%>
