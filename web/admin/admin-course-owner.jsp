...
if (!adminCourse.isNew())
{
    try
    {
        PersonBean owner = adminCourse.getOwnerPerson();
        if (owner != null)
        {
%>
    document.coursePeopleForm.ownerSelect.value = '<%= owner.getIdString() %>';
<%
        }
    }
    catch (Exception x)
...
%>
    document.coursePeopleForm.contactNameInput.value = '<%= adminCourse.getContactNameString() %>';
    document.coursePeopleForm.contactEmailInput.value = '<%= adminCourse.getContactEmailString() %>';
    document.coursePeopleForm.contactDescInput.value = '<%= adminCourse.getFulfillmentEmailString() %>';
<%
}
%>
}
...
<TABLE WIDTH=772 BORDER=0 CELLPADDING=0 CELLSPACING=0>
    <struts-html:form action="/admin/coursePeople">
	<TR>
...
<%@ include file="channels\channel-course-menu.jsp" %>
			...
			<TABLE WIDTH=522 BORDER=0 CELLPADDING=1 CELLSPACING=0 BGCOLOR="#6F9BCB">
				<TR HEIGHT=26>
					<TD ALIGN=left BACKGROUND="../images/PttrsnUK_TableHeaderBG3.jpg"><P style="font-size: 13pt; font-family: Arial; font-weight: bold; color: #000000;">&nbsp;Course Owner</P></TD>
				</TR>
				<TR>
					<TD>
							...
							<TR HEIGHT=30>
								...
								<TD>
                                                                    <struts-html:select property="ownerSelect" style="font-size: 13px; width: 410px;" >
                                                                        <struts-html:option value="0">-- SELECT AN OWNER FOR THIS COURSE --</struts-html:option>
			...
			<TABLE WIDTH=522 BORDER=0 CELLPADDING=1 CELLSPACING=0 BGCOLOR="#6F9BCB">
				<TR HEIGHT=26>
					<TD ALIGN=left BACKGROUND="../images/PttrsnUK_TableHeaderBG3.jpg"><P style="font-size: 13pt; font-family: Arial; font-weight: bold; color: #000000;">&nbsp;Course Contact(s)</P></TD>
				</TR>
				<TR>
					<TD>
						<TABLE WIDTH=520 BORDER=0 CELLPADDING=1 CELLSPACING=0 BGCOLOR="#FFFFFF">
							...
							<TR>
								...
								<TD ALIGN=left><P style="font-size: 9pt; font-family: Arial; color: #000000;"><A HREF="mailto:<%= adminCourse.getContactEmailString() %>" style="color: #003366"><%= adminCourse.getContactNameString() %></A></P></TD>
								<TD ALIGN=left><P style="font-size: 9pt; font-family: Arial; color: #000000;"><%= adminCourse.getFulfillmentEmailString() %></P></TD>
