 ...
 * @author marlo
 */
public class QBXMLCustomerModRequest
		...
		throws TorqueException, ObjectNotFoundException
    {
		...
		if (qb_request == null)
		{
			...
			qb_request = QBXMLCustomerModRequest.getRequest((QbRequestDb)objList.get(0));
		}
		...
		throws TorqueException
    {
		...
		if (qb_request == null)
		{
			qb_request = new QBXMLCustomerModRequest(_request);
    ...
    public
    QBXMLCustomerModRequest(QbRequestDb _request)
    {
		qb_request = _request;
		isNew = false;
    }
	...
	protected void assembleRequestBody() throws WritingException {
		...
		xmlwriter.writeEntity("CustomerModRq");
		xmlwriter.writeAttribute("requestID", getRequestId());

		xmlwriter.writeEntity("CustomerMod");

		try
		{
			xmlwriter.writeEntity("ListID");
			xmlwriter.writeText(customer.getQBFSListID());
			xmlwriter.endEntity();
		}
		...
		try
		{
			CustomerRet obj = CustomerRet.getCustomerQBFS(customer.getSelectedCompany(), customer.getQBFSListID());

			if (obj.getEditSequence() != null)
			{
				xmlwriter.writeEntity("EditSequence");
				xmlwriter.writeText(obj.getEditSequence());
				xmlwriter.endEntity();
			}
		}
		catch (Exception x)
		...
		xmlwriter.endEntity(); // </CustomerMod>

		xmlwriter.endEntity(); // </CustomerModRq>

	}

	@Override
	public String getLabel() {
		return CUBean.getUserTimeString(super.creation_date) + " - Customer Mod Request";
	}

	@Override
	public String getRequestType() {
		return "CustomerModRq";
	}
