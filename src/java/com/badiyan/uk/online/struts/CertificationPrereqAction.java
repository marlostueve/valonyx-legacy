	...
	extends Action
{
	...
	execute(ActionMapping _mapping, ActionForm _form,
		...
		throws Exception
	{
		System.out.println("execute() invoked in CertificationPrereqAction");
		...
		try
		{
			...
			if (typeSelect.intValue() == 0)
			{
				CourseBean prereqCourse = CourseBean.getCourse(Integer.parseInt(titleSelect));
				certBean.addPrerequisite(prereqCourse);
			}
			else
			{
				CertificationBean prereqCert = CertificationBean.getCertification(Integer.parseInt(titleSelect));
				certBean.addPrerequisite(prereqCert);
			}
		}
        catch (IllegalValueException x)
        {
