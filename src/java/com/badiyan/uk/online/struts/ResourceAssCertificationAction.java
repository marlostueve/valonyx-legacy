 ...
 */
public final class
ResourceAssCertificationAction
	extends Action
{
	...
	execute(ActionMapping _mapping, ActionForm _form,
		...
		throws Exception
	{
		System.out.println("execute() invoked in ResourceAssCertificationAction");
		...
		try
		{
			...
			if (session != null)
			{
				...
				if (resourceBean == null)
				{
					...
					session.setAttribute("adminResource", resourceBean);
				}
			...
			// I need to create a new association between the

			if (!titleSelect.equals("0"))
			{
				CertificationBean certification = CertificationBean.getCertification(Integer.parseInt(titleSelect));
				resourceBean.associate(certification);
