	...
	extends Action
{
	...
	execute(ActionMapping _mapping, ActionForm _form,
		...
		throws Exception
	{
		System.out.println("execute() invoked in CommissionReportFilterAction");
		...
		try
		{
			...
			if (practitionerSelect != null && practitionerSelect.intValue() > 0)
			{
			...
			else if (practitionerSelect != null && practitionerSelect.intValue() == -1)
				courseReportLister.setPerson(new UKOnlinePersonBean()); // not ideal, but I'll use this to represent the "No Practitioners" selection, for now
			else
