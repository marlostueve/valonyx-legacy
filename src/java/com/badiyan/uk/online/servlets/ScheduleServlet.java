    ...
    extends HttpServlet
{
		...
		throws ServletException, IOException
    {
		...
		try
		{
			...
			*/
			...
			{
				System.out.println("refresh stuff >");
				...
				Boolean needs_update = (Boolean)ScheduleServlet.needs_update_hash.get(session_str);
				if ((needs_update == null) || !needs_update.booleanValue())
					refresh_tag_name = (ScheduleServlet.last_update_hash.get(key) == null) ? "status" : "refresh";
				else
				{
					System.out.println("NEEDS UPDATE!!!");
					refresh_tag_name = needs_update.booleanValue() ? "status" : "refresh";
					get_all_appointments = true;
					ScheduleServlet.needs_update_hash.remove(session_str);
				}

				String return_str = null;
				if ((parameter != null) && parameter.equals("p"))
					return_str = "<" + refresh_tag_name + ">" + this.assemblePractitionerScheduleXML(adminCompany, adminPractitioner, display_date, session_str, get_all_appointments).toString() + "</" + refresh_tag_name + ">";
				else
			...
			else if (command.equals("forceRefresh"))
			{
				...
				String return_str = null;
				if ((parameter != null) && parameter.equals("p")) {
					return_str = "<status>" + this.assemblePractitionerScheduleXML(adminCompany, adminPractitioner, display_date, session_str, true).toString() + "</status>";
				} else {
					return_str = "<status>" + this.assembleScheduleXML(adminCompany, adminPracticeArea, display_date, session_str, true).toString() + "</status>";
				}
				System.out.println("return_str (2) >" + return_str);
				writer.println(return_str);
			}
			else if (command.equals("newAppointment"))
			{
				...
				if ((parameter != null) && parameter.equals("p"))
					writer.println("<status>" + this.assemblePractitionerScheduleXML(adminCompany, adminPractitioner, display_date, session_str, true).toString() + "</status>");
				else
			...
			else if (command.equals("gotoDay"))
			{
				...
				try
				{
					display_date.setTime(ScheduleServlet.ie_date_format.parse(parameter));
				}
				catch (java.text.ParseException x)
				{
					display_date.setTime(ScheduleServlet.ns_date_format.parse(parameter.substring(0, parameter.indexOf(':'))));
				}
			...
			else if (command.equals("gotoDayP"))
			{
				Calendar display_date = (Calendar)session.getAttribute("display_date");
				...
				StringBuffer returnBuf = this.assemblePractitionerScheduleXML(adminCompany, adminPractitioner, display_date, session_str, true);
				
				String date_str = ScheduleServlet.date_format.format(display_date.getTime());
				String key = session_str + "" + ScheduleServlet.date_format.format(display_date.getTime()) + "p";
			...
			else if (command.equals("nextWeek"))
			{
				Calendar display_date = (Calendar)session.getAttribute("display_date");
				display_date.add(Calendar.DATE, 7);
				
				StringBuffer returnBuf = this.assemblePractitionerScheduleXML(adminCompany, adminPractitioner, display_date, session_str, true);

				writer.println("<status>" + returnBuf.toString() + "</status>");
			}
			else if (command.equals("previousWeek"))
			{
				Calendar display_date = (Calendar)session.getAttribute("display_date");
				display_date.add(Calendar.DATE, -7);
			...
			else if (command.equals("getCheckoutCodesByDesc"))
			{
				...
				long number_parameter = -1;
				try {
					number_parameter = Long.parseLong(parameter);
					is_numeric = true;
				} catch (Exception x) {	}

				Vector codes_vec = null;
				if (is_numeric) {
					codes_vec = CheckoutCodeBean.getCheckoutCodes(adminCompany, number_parameter);
				} else {
					codes_vec = CheckoutCodeBean.getCheckoutCodes(adminCompany, parameter);
				}
				
				Iterator code_itr = codes_vec.iterator();
				while (code_itr.hasNext())
				{
					...
					if (code.isSynced() || !adminCompany.getQuickBooksSettings().isQuickBooksFSEnabled())
						buf.append(ScheduleServlet.getXML(code));
				}

				if (is_numeric && (codes_vec.size() == 1)) {
					writer.println("<codes add=\"true\">" + buf.toString() + "</codes>");
				} else {
			...
			else if (command.equals("nextAppt") || command.equals("nextApptP"))
			{
				...
				{
					returnBuf = this.assemblePractitionerScheduleXML(adminCompany, adminPractitioner, display_date, session_str, true);
					key = session_str + "" + date_str + "p";
				}
				...
				String return_str = null;
				if (command.equals("nextAppt"))
					return_str = str1 + getXML(next_available_appt) + str2;
				else
					return_str = str1 + getPractitionerXML(next_available_appt) + str2;
				
				writer.println("<status>" + return_str + "</status>");

				ScheduleServlet.needs_update_hash.put(session_str, new Boolean(false));
			}
			else if (command.equals("offTime"))
			{
				...
				returnBuf.append("<date>" + ScheduleServlet.date_format.format(display_date.getTime()) + "</date>");
			...
			else if (command.equals("checkin") || command.equals("checkinP"))
			{
				...
				while (itr.hasNext())
				{
					...
					{
						has_alert = true;
						buf.append(ScheduleServlet.getXMLDetail(note_obj));
					}
				}
				buf.append("</alerts>");
				...
				if (command.equals("checkin"))
					...
					writer.println("<refresh>" + this.assemblePractitionerScheduleXML(adminCompany, adminPractitioner, (Calendar)session.getAttribute("display_date"), session_str, false).toString() + "</refresh>");
			}
			else if (command.equals("checkout") || command.equals("checkoutP"))
			{
				AppointmentBean selected_appointment = AppointmentBean.getAppointment(Integer.parseInt(parameter));
			...
			else if (command.equals("reschedule") || command.equals("rescheduleP"))
			{
				AppointmentBean selected_appointment = AppointmentBean.getAppointment(Integer.parseInt(parameter));
			...
			else if (command.equals("cancel") || command.equals("cancelP"))
			{
				AppointmentBean selected_appointment = AppointmentBean.getAppointment(Integer.parseInt(parameter));
			...
			else if (command.equals("clear") || command.equals("clearP"))
			{
				AppointmentBean selected_appointment = AppointmentBean.getAppointment(Integer.parseInt(parameter));
			...
			else if (command.equals("getCheckoutDetails") || command.equals("getCheckoutDetailsCID"))
			{
				...
				while (open_orders_itr.hasNext())
				{
					...
					//open_orders_buf.append(ScheduleServlet.getXML(open_order_obj));
				...
				if (client.hasQBFSListID())
				{
					...
					while (itr.hasNext())
					{
						...
						open_orders_buf.append(ScheduleServlet.getXML(invoice_obj, adminCompany));
					}
				}
			...
			else if (command.equals("addClientReview"))
			{
				...
				if (practitioner_id > 0) {
					...
					vec.addElement((UKOnlinePersonBean)UKOnlinePersonBean.getPerson(practitioner_id));
					client_review_reason.setPractitioners(vec);
				}
			...
			else if (command.equals("paste") || command.equals("pastep"))
			{
				...
				if (selected_appointment != null)
				{
					...
					{
						StringBuffer returnBuf = this.assemblePractitionerScheduleXML(adminCompany, adminPractitioner, paste_date, session_str, true);
						writer.println("<status>" + returnBuf.toString() + "</status>");
					}
				}
			}
			else if (command.equals("delete") || command.equals("deleteP"))
			{
				...
				boolean is_client_appt = delete_appointment.isClientAppointment();
				...
				if (is_client_appt) {
					try {
						...
						String appt_id_str = delete_appointment.getValue();
						String client_name_str = delete_appointment.getClient().getLabel();

						System.out.println("deleting appt");
						System.out.println("appt_id_str >" + appt_id_str);
						System.out.println("client_name_str >" + client_name_str);

						UKOnlineLoginBean loginBean = (UKOnlineLoginBean)session.getAttribute("loginBean");

						if (!client_name_str.equals("Christine Stueve")) {
							CUBean.sendEmail("marlo@badiyan.com", "marlo@valonyx.com", "An Appointment had been Deleted by " + loginBean.getPerson().getLabel(), "appt_id_str >" + appt_id_str + ",    " + "client_name_str >" + client_name_str);
						}
					} catch (Exception x) {
						//x.printStackTrace();
					}
			...
			{
				AppointmentBean delete_appointment = AppointmentBean.getAppointment(Integer.parseInt(parameter));
				delete_appointment.setState(AppointmentBean.DELETED_APPOINTMENT_STATUS);

				String deleted_appointment_XML_str = null;
				if (command.equals("delete"))
					deleted_appointment_XML_str = ScheduleServlet.getXML(delete_appointment);
				else
					deleted_appointment_XML_str = ScheduleServlet.getPractitionerXML(delete_appointment);

				String conflict_XML_str = "";
				Vector conflicts = AppointmentBean.getConflicts(delete_appointment);
				System.out.println("num conflicts >" + conflicts.size());
				Iterator itr = conflicts.iterator();
				while (itr.hasNext())
				{
					AppointmentBean conflict_appointment = (AppointmentBean)itr.next();
					conflict_XML_str += ScheduleServlet.getXML(conflict_appointment);
				}
				...
				returnBuf.append("<schedule>");
				returnBuf.append(deleted_appointment_XML_str);
				returnBuf.append("</schedule>");

				System.out.println("DELETE BUF >" + returnBuf.toString());

				writer.println("<refresh>" + returnBuf.toString() + "</refresh>");
			...
			else if (command.equals("delete") || command.equals("deleteP"))
			{
				AppointmentBean selected_appointment = AppointmentBean.getAppointment(Integer.parseInt(parameter));
				session.setAttribute("adminPerson", selected_appointment.getClient());
				selected_appointment.setState(AppointmentBean.DELETED_APPOINTMENT_STATUS);
				selected_appointment.save();

				if (selected_appointment.hasParent())
				{
					AppointmentBean parent_appointment = selected_appointment.getParent();
					parent_appointment.invalidateChildren();
				}

				String str = null;

				if (command.equals("delete"))
					str = "<refresh>" + this.assembleScheduleXML(adminCompany, adminPracticeArea, (Calendar)session.getAttribute("display_date"), session_str, false).toString() + "</refresh>";
				else
					str = "<refresh>" + this.assemblePractitionerScheduleXML(adminCompany, adminPractitioner, (Calendar)session.getAttribute("display_date"), session_str, false).toString() + "</refresh>";

				System.out.println("DELETE STR >" + str);
				writer.println(str);
			}
			 */
			...
			else if (command.equals("salesReceipt"))
			{
				...
				String sales_receipt_url = "../resources/pdf/" + SalesReceiptBuilder.generateSalesReceipt(adminCompany, (UKOnlinePersonBean)adminLoginBean.getPerson(), client, adminOrder, previous_orders, tenders, subtotal, tax, total, total_discount_products, total_discount_services, discount_perc_products, discount_perc_services, shipping);
			...
			else if (command.equals("lastReceipt"))
			{
				...
				String sales_receipt_url = "../resources/pdf/" + SalesReceiptBuilder.generateSalesReceipt(adminCompany, (UKOnlinePersonBean)adminLoginBean.getPerson(), client, primary_order, previous_orders, tenders, subtotal, tax, total, total_discount_products, total_discount_services, discount_perc_products, discount_perc_services, shipping);
				writer.println("<sales_receipt_url><![CDATA[" + sales_receipt_url + "]]></sales_receipt_url>");
			}
			else if (command.equals("blackHole"))
			{
				...
				try
				{
					...
					while (members_itr.hasNext())
					{
						...
						returnBuf.append(ScheduleServlet.getXML(member_obj));
					}
				}
			...
			else if (command.equals("clientReview"))
			{
				...
				while (items_itr.hasNext())
				{
					...
					returnBuf.append(ScheduleServlet.getXMLWithClientInfo(obj));
				}
			...
			else if (command.equals("pracList"))
			{
				...
				try
				{
					...
					while (call_list_itr.hasNext())
					{
						...
						returnBuf.append(ScheduleServlet.getXMLWithClientInfo(contact_obj));
					}
				}
				catch (java.util.ConcurrentModificationException x)
				{
			...
			else if (command.equals("gotoLastApptP") || command.equals("gotoNextApptP"))
			{
				UKOnlinePersonBean person = (UKOnlinePersonBean)UKOnlinePersonBean.getPerson(Integer.parseInt(parameter));
				session.setAttribute("adminPerson", person);

				AppointmentBean appt = null;
				if (command.equals("gotoNextApptP"))
					appt = AppointmentBean.getNextAppointmentForClient(person);
				else
					appt = AppointmentBean.getLastAppointmentForClient(person);
			...
			else if (command.equals("saveSoap"))
			{
				...
				if (!note.getTimeInStr().isEmpty()) {
					str += "\r\nTime In: " + note.getTimeInStr();
				}
				if (!note.getTimeOutStr().isEmpty()) {
					str += "\r\nTime Out: " + note.getTimeOutStr();
				}
				note.setONote(str);
				note.save();
			}
			...
			{
				SOAPNotesBean note = (SOAPNotesBean)session.getAttribute("soapNote");
				note.setTimeInStr(parameter);
			}
			else if (command.equals("saveTimeOut"))
			{
				SOAPNotesBean note = (SOAPNotesBean)session.getAttribute("soapNote");
				note.setTimeOutStr(parameter);
			}
			...
			else if (command.equals("showNote"))
			{
				...
				writer.println("<note>" + ScheduleServlet.getXMLDetail(note_obj) + "</note>");
			}
			else if (command.equals("showSoap"))
			{
				SOAPNotesBean note_obj = SOAPNotesBean.getSOAPNotes(Integer.parseInt(parameter));
				writer.println("<soap>" + ScheduleServlet.getXMLDetail(note_obj) + "</soap>");
			}
			...
			else if (command.equals("printSoap"))
			{
				...
				String soap_report_url = "../resources/pdf/" + SOAPNoteReportBuilder.generateSOAPNote(adminCompany, note.getPerson(), note.getCreatePerson(), note.getCreationDate(), note);
			...
			else if (command.equals("updateQuickBooks"))
			{
				...
				System.out.println("cash_out.isQuickBooksUpdateInProgress() >" + cash_out.isQuickBooksUpdateInProgress());

				//if (!cash_out.isQuickBooksUpdateInProgress()) {
				if (cash_out.isAllow_regen_stuff()) {
					cash_out.setAllow_regen_stuff(false);
			...
			else if (command.equals("selectReviewItem"))
			{
				...
				ClientReviewReason client_review_reason = ClientReviewReason.getClientReviewReason(Integer.parseInt(parameter));
				writer.println(ScheduleServlet.getXMLDetail(client_review_reason));
		...
		throws Exception
	{
		...
		String date_str = ScheduleServlet.date_format.format(_display_date.getTime());
		...
		{
			Date last_update = ScheduleServlet.last_update_hash.get(key);
		...
		{
			AppointmentBean appointment = (AppointmentBean)itr.next();
			returnBuf.append(ScheduleServlet.getXML(appointment));
		}
	...
	private StringBuffer
	assemblePractitionerScheduleXML(UKOnlineCompanyBean _company, UKOnlinePersonBean _practitioner, Calendar _display_date, String _session_id, boolean _get_all_appts)
		throws Exception
	{
		//System.out.println("assemblePractitionerScheduleXML() invoked");
		...
		Date display_date = _display_date.getTime();
		Calendar start_of_week = Calendar.getInstance();
		start_of_week.setTime(display_date);
		Calendar end_of_week = Calendar.getInstance();
		end_of_week.setTime(display_date);
		...
		Vector late = new Vector();
		Vector no_show = new Vector();
		Iterator appt_itr = AppointmentBean.getAppointments(_company, _practitioner, start_of_week.getTime(), end_of_week.getTime()).iterator();
		while (appt_itr.hasNext())
		{
			AppointmentBean appointment = (AppointmentBean)appt_itr.next();
			if (appointment.isClientAppointment())
			{
				...
				{
					case AppointmentBean.CANCELLED_APPOINTMENT_STATUS: num_cancelled++; break;
					case AppointmentBean.CHECKED_IN_APPOINTMENT_STATUS: checked_in.addElement(appointment); break;
					case AppointmentBean.CHECKED_OUT_APPOINTMENT_STATUS: num_checked_out++; break;
					case AppointmentBean.DEFAULT_APPOINTMENT_STATUS: break;
					case AppointmentBean.LATE_APPOINTMENT_STATUS: late.addElement(appointment); break;
					case AppointmentBean.NO_SHOW_APPOINTMENT_STATUS: no_show.addElement(appointment); break;
					case AppointmentBean.RESCHEDULE_APPOINTMENT_STATUS:num_rescheduled++; break;
		...
		//String date_str = ScheduleServlet.date_format.format(_display_date.getTime());
		String date_str = ScheduleServlet.date_format.format(start_of_week.getTime()) + " - " + ScheduleServlet.date_format.format(end_of_week.getTime());
		returnBuf.append("<date tot=\"" + num_total + "\" cn=\"" + num_cancelled + "\" co=\"" + num_checked_out + "\" rs=\"" + num_rescheduled + "\">" + date_str + "</date>");
		returnBuf.append("<schedule>");

		String key = _session_id + "" + ScheduleServlet.date_format.format(_display_date.getTime()) + "p";

		String appt_str = null;
		if (_get_all_appts)
			appt_str = this.getPractitionerScheduleXML(_company, _practitioner, start_of_week.getTime(), end_of_week.getTime());
		else
		{
			Date last_update = ScheduleServlet.last_update_hash.get(key);
			...
			if (last_update == null)
				...
				appt_str = this.getPractitionerScheduleXML(_company, _practitioner, start_of_week.getTime(), end_of_week.getTime(), last_update);
    ...
    private String
    getScheduleXML(UKOnlineCompanyBean _company, PracticeAreaBean _practice_area, Date _date)
		throws Exception
    {
		...
		Iterator appointment_itr = AppointmentBean.getAppointments(_company, _practice_area, _date).iterator();
		while (appointment_itr.hasNext())
		{
			...
			if (appointment.isCancelled() || appointment.isRescheduled())
				cancelled_rescheduled_buf.append(ScheduleServlet.getXML(appointment));
			else
			{
				AppointmentTypeBean type = appointment.getType();
				if (type.getType() == AppointmentTypeBean.CLIENT_APPOINTMENT_TYPE_TYPE)
					appt_buf.append(ScheduleServlet.getXML(appointment));
				else if (type.getType() == AppointmentTypeBean.MEETING_APPOINTMENT_TYPE_TYPE)
					meeting_buf.append(ScheduleServlet.getXML(appointment));
				else
					off_time_buf.append(ScheduleServlet.getXML(appointment));
			}
		}
    ...
    private String
    getScheduleXML(CompanyBean _company, PracticeAreaBean _practice_area, Date _date, Date _last_update_date)
		throws Exception
    {
		StringBuffer appt_buf = new StringBuffer();
		StringBuffer off_time_buf = new StringBuffer();
		...
		HashMap appointments = AppointmentBean.getAppointments(_company, _practice_area, _date, _last_update_date);
		...
		throws Exception
    {
		...
		while (appointment_itr.hasNext())
		{
			...
			if (appointment.isCancelled() || appointment.isRescheduled())
				cancelled_rescheduled_buf.append(ScheduleServlet.getPractitionerXML(appointment));
			else
			{
				AppointmentTypeBean type = appointment.getType();
				if (type.getType() == AppointmentTypeBean.CLIENT_APPOINTMENT_TYPE_TYPE)
					appt_buf.append(ScheduleServlet.getPractitionerXML(appointment));
				else if (type.getType() == AppointmentTypeBean.MEETING_APPOINTMENT_TYPE_TYPE)
					meeting_buf.append(ScheduleServlet.getPractitionerXML(appointment));
				else
					off_time_buf.append(ScheduleServlet.getPractitionerXML(appointment));
			}
		}
		...
		throws Exception
    {
		...
		while (keys.hasNext())
		{
			...
			while (appointment_itr.hasNext())
			{
				...
				buf.append(ScheduleServlet.getXML(appointment));
			}
		}
