    ...
    extends TimerTask
{
	...
	private QBMSCreditCardResponse charge_to_void = null;
	private TenderRet tender_to_void = null;
    ...
    public
    VoidChargeTask(UKOnlineCompanyBean _company, HttpSession _session, QBMSCreditCardResponse _charge_to_void)
    {
		//if (CUBean.getProperty("qbms.isProduction") != null)
		//	is_production = CUBean.getProperty("qbms.isProduction").equals("true");

		charge_to_void = _charge_to_void;

		session = _session;
		//status = (StringBuffer)session.getAttribute("void_status");
		
		company = _company;
    }

    public
    VoidChargeTask(UKOnlineCompanyBean _company, HttpSession _session, TenderRet _tender_to_void)
		throws TorqueException, ObjectNotFoundException
    {
		//if (CUBean.getProperty("qbms.isProduction") != null)
		//	is_production = CUBean.getProperty("qbms.isProduction").equals("true");

		tender_to_void = _tender_to_void;
		charge_to_void = QBMSCreditCardResponse.getResponse(tender_to_void);
    ...
    run()
    {
		System.out.println("run() invoked in VoidChargeTask");
		...
		try
		{
			...
			try
			{
				...
				post_values.put("x_ref_trans_id", charge_to_void.getCreditCardTransId());
				...
				System.out.println("void response_code >" + response_code);
				System.out.println("void error_code >" + error_code);

				if (response_code == 1)
				{
					QBMSCreditCardResponse response = new QBMSCreditCardResponse();
					response.setClient(charge_to_void.getClient());
					...
					if (response.getCreditCardTransId() == null || response.getCreditCardTransId().equals("0"))
						response.setCreditCardTransId(charge_to_void.getCreditCardTransId());
					response.setResponseCode((short)response_code);
					response.setErrorCode((short)error_code);
					response.save();


					if (tender_to_void != null)
					{
						...
						tender_to_void.setVoidedOrRefunded();
				...
				{
					CUBean.sendEmail("marlo@valeowc.com", "admin@valeowc.com", "Void Error: " + parser.getErrorText(), "response_code == 2 - declined");
				...
				{
					CUBean.sendEmail("marlo@valeowc.com", "admin@valeowc.com", "Void Error: " + parser.getErrorText(), "response_code == 3 - error");
				...
				{
					CUBean.sendEmail("marlo@valeowc.com", "admin@valeowc.com", "Void Error: " + parser.getErrorText(), "response_code == 4 - being held for review?!?!");
