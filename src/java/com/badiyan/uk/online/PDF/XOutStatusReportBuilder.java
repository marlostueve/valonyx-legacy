...
XOutStatusReportBuilder
{
		...
		throws IllegalValueException
	{
		...
		String file = "x-out-status-report-" + System.currentTimeMillis() + ".pdf";
		...
		try
		{
			...
			BigDecimal product_sales = CUBean.zero;
			BigDecimal service_sales = CUBean.zero;
			BigDecimal other_sales = CUBean.zero; // shrug
			...
			BigDecimal lab_in = CUBean.zero;
			BigDecimal lab_out = CUBean.zero; // example?
			...
			Vector lab_ids = new Vector();
			if (_company.getValue().equals("1"))
			{
				System.out.println("ADDING LAB IDs for Valeo");
				lab_ids.addElement(new Integer(912));
				lab_ids.addElement(new Integer(913));
			}
			...
			while (orders_itr.hasNext())
			{
				...
				while (orderline_itr.hasNext()) {
					...
					Integer checkout_code_id = new Integer(orderline.getCheckoutCodeId());
					System.out.println("orderline id >" + orderline.getCheckoutCodeId());
					
					BigDecimal tax = orderline.getTax();
					BigDecimal total  = orderline.getPricePrecise().multiply(orderline.getQuantity());
					//BigDecimal total  = orderline.
					...
					if (order.isReturn()) {
						...
						if (lab_ids.contains(checkout_code_id)) {
							lab_out = lab_out.add(total);
						}
						...
						System.out.println("NOT A RETURN (and not reversed) >" + total.toString());
						...
						try {
							CheckoutCodeBean checkout_code = CheckoutCodeBean.getCheckoutCode(checkout_code_id);
							if (checkout_code.getType() == CheckoutCodeBean.INVENTORY_TYPE) {
								System.out.println("INVENTORY_TYPE >" + checkout_code.getLabel());
								product_sales = product_sales.add(total);
							} else if (checkout_code.getType() == CheckoutCodeBean.PROCEDURE_TYPE) {
								System.out.println("PROCEDURE_TYPE >" + checkout_code.getLabel());
								service_sales = service_sales.add(total);
							} else {
								System.out.println("TYPE " + checkout_code.getTypeString() + " >" + checkout_code.getLabel());
								other_sales = other_sales.add(total);
							}
						} catch (Exception x) {
							x.printStackTrace();
						}
			...
			if (lab_in.compareTo(BigDecimal.ZERO) != 0 || lab_out.compareTo(BigDecimal.ZERO) != 0)
				height_offset += (3 * height_per_row);
			...
			XOutStatusReportBuilder.doText(CUBean.getUserDateString(now.getTime()), cb, bf, 8, height, row, left_margin);
			XOutStatusReportBuilder.doText(CUBean.getUserTimeString(now.getTime()), cb, bf, 8, height, row, 195f); row ++;

			XOutStatusReportBuilder.doText("X-Out Status Report", cb, bf, 12, height, row, 60f); row++;
			XOutStatusReportBuilder.doText("Date " + CUBean.getUserDateString(_cash_out.getStartDate()) + " 12:00 AM to " + CUBean.getUserDateString(_cash_out.getEndDate()) + " 11:59 PM - " + _workstation, cb, bf, 6, height, row, left_margin); row++;
			XOutStatusReportBuilder.doText("Sales Activity", cb, bf, 9, height, row, left_margin);
			XOutStatusReportBuilder.doText("Sales", cb, bf, 9, height, row, 80f);
			XOutStatusReportBuilder.doText("Returns", cb, bf, 9, height, row, 135f);
			XOutStatusReportBuilder.doText("Net", cb, bf, 9, height, row, 200f); row++;

			
			
			XOutStatusReportBuilder.doText("Non Txbl :", cb, bf, 8, height, row, left_margin);

			String str = XOutStatusReportBuilder.formatCurrency(non_taxable_sales);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 80f));
			str = XOutStatusReportBuilder.formatCurrency(non_taxable_returns);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 145f));
			str = XOutStatusReportBuilder.formatCurrency(non_taxable_net);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Txbl :", cb, bf, 8, height, row, left_margin);
			str = XOutStatusReportBuilder.formatCurrency(taxable_sales);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 80f));
			str = XOutStatusReportBuilder.formatCurrency(taxable_returns);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 145f));
			str = XOutStatusReportBuilder.formatCurrency(taxable_net);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Tax :", cb, bf, 8, height, row, left_margin);
			str = XOutStatusReportBuilder.formatCurrency(sales_tax);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 80f));
			str = XOutStatusReportBuilder.formatCurrency(return_tax);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 145f));
			str = XOutStatusReportBuilder.formatCurrency(tax_net);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Subtotal :", cb, bf, 8, height, row, left_margin);
			str = XOutStatusReportBuilder.formatCurrency(non_taxable_sales.add(taxable_sales.add(sales_tax)));
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 80f));
			str = XOutStatusReportBuilder.formatCurrency(non_taxable_returns.add(taxable_returns.add(return_tax)));
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 145f));
			str = XOutStatusReportBuilder.formatCurrency(non_taxable_net.add(taxable_net.add(tax_net)));
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			...
			XOutStatusReportBuilder.doText("Payments :", cb, bf, 8, height, row, left_margin);
			str = XOutStatusReportBuilder.formatCurrency(payments);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 80f));
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Tips :", cb, bf, 8, height, row, left_margin);
			str = XOutStatusReportBuilder.formatCurrency(CUBean.zero);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 80f));
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Shipping :", cb, bf, 8, height, row, left_margin);
			str = XOutStatusReportBuilder.formatCurrency(CUBean.zero);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 80f));
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			...
			XOutStatusReportBuilder.doText("Total Activity :", cb, bf, 8, height, row, left_margin);
			str = XOutStatusReportBuilder.formatCurrency(non_taxable_sales.add(taxable_sales.add(sales_tax.add(payments))));
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 80f));
			str = XOutStatusReportBuilder.formatCurrency(non_taxable_returns.add(taxable_returns.add(return_tax)));
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 145f));
			BigDecimal net_activity = non_taxable_net.add(taxable_net.add(tax_net.add(payments)));
			str = XOutStatusReportBuilder.formatCurrency(net_activity);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			...
			*/
			...
			try {
				XOutStatusReportBuilder.doText("Product Sales :", cb, bf, 8, height, row, left_margin);
				str = XOutStatusReportBuilder.formatCurrency(product_sales);
				XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;		

				XOutStatusReportBuilder.doText("Service Sales :", cb, bf, 8, height, row, left_margin);
				str = XOutStatusReportBuilder.formatCurrency(service_sales);
				XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

				XOutStatusReportBuilder.doText("Other Sales :", cb, bf, 8, height, row, left_margin);
				str = XOutStatusReportBuilder.formatCurrency(other_sales);
				XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			...
			XOutStatusReportBuilder.doText("Sales Adjustments", cb, bf, 9, height, row, left_margin); row++;

			XOutStatusReportBuilder.doText("Net Sales Activity :", cb, bf, 8, height, row, left_margin);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Charged to account :", cb, bf, 8, height, row, left_margin);
			str = XOutStatusReportBuilder.formatCurrency(account_activity);
			XOutStatusReportBuilder.doText("" + str + "", cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			/*
			XOutStatusReportBuilder.doText("Gift Certificates Used :", cb, bf, 8, height, row, left_margin);
			str = XOutStatusReportBuilder.formatCurrency(gift_certificate_activity);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Gift Cards Used :", cb, bf, 8, height, row, left_margin);
			str = XOutStatusReportBuilder.formatCurrency(gift_card_activity);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			*/

			XOutStatusReportBuilder.doText("Net Deposits Used :", cb, bf, 8, height, row, left_margin); // where does this come from??? and, does it subtract from the total???
			str = XOutStatusReportBuilder.formatCurrency(net_deposits);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Payouts :", cb, bf, 8, height, row, left_margin); // where does this come from???
			str = XOutStatusReportBuilder.formatCurrency(payouts);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Total Available for Deposit :", cb, bf, 9, height, row, left_margin);
			...
			str = XOutStatusReportBuilder.formatCurrency(total_available_for_deposit);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			row++;

			XOutStatusReportBuilder.doText("Discounts", cb, bf, 9, height, row, left_margin);

			XOutStatusReportBuilder.doText("Sales", cb, bf, 9, height, row, 80f);
			XOutStatusReportBuilder.doText("Returns", cb, bf, 9, height, row, 135f);
			XOutStatusReportBuilder.doText("Net", cb, bf, 9, height, row, 200f); row++;

			XOutStatusReportBuilder.doText("Discount :", cb, bf, 8, height, row, left_margin);
			str = XOutStatusReportBuilder.formatCurrency(sales_discount);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 80f));
			str = XOutStatusReportBuilder.formatCurrency(return_discount);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 145f));
			str = XOutStatusReportBuilder.formatCurrency(sales_discount.subtract(return_discount));
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			row++;


			XOutStatusReportBuilder.doText("Receipt Counts", cb, bf, 9, height, row, left_margin);

			XOutStatusReportBuilder.doText("Sales :", cb, bf, 8, height, row, 100f);
			str = num_sales_receipts + "";
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Returns :", cb, bf, 8, height, row, 100f);
			str = num_return_receipts + "";
			XOutStatusReportBuilder.doText(num_return_receipts + "", cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Deposits :", cb, bf, 8, height, row, 100f); // not sure where this comes from
			str = num_deposit_receipts + "";
			XOutStatusReportBuilder.doText(num_deposit_receipts + "", cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Reversed :", cb, bf, 8, height, row, 100f); // what's the difference between a reverse and a return??? assuming reverse only possible on the same day or something...
			str = num_reversed_receipts + "";
			XOutStatusReportBuilder.doText(num_reversed_receipts + "", cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Payouts :", cb, bf, 8, height, row, 100f); // not sure where this comes from
			str = num_payout_receipts + "";
			XOutStatusReportBuilder.doText(num_payout_receipts + "", cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			row++;

			XOutStatusReportBuilder.doText("Dollars", cb, bf, 9, height, row, left_margin);
			XOutStatusReportBuilder.doText("Paid In :", cb, bf, 8, height, row, 100f);
			str = XOutStatusReportBuilder.formatCurrency(cash_in);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			XOutStatusReportBuilder.doText("Paid Out :", cb, bf, 8, height, row, 100f);
			str = XOutStatusReportBuilder.formatCurrency(cash_out);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			XOutStatusReportBuilder.doText("Net :", cb, bf, 8, height, row, 100f);
			str = XOutStatusReportBuilder.formatCurrency(cash_in.subtract(cash_out));
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Check", cb, bf, 9, height, row, left_margin);
			XOutStatusReportBuilder.doText("Paid In :", cb, bf, 8, height, row, 100f);
			str = XOutStatusReportBuilder.formatCurrency(check_in);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			XOutStatusReportBuilder.doText("Paid Out :", cb, bf, 8, height, row, 100f);
			str = XOutStatusReportBuilder.formatCurrency(check_out);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			XOutStatusReportBuilder.doText("Net :", cb, bf, 8, height, row, 100f);
			str = XOutStatusReportBuilder.formatCurrency(check_in.subtract(check_out));
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("All Credit/Debit Cards", cb, bf, 9, height, row, left_margin);
			XOutStatusReportBuilder.doText("Paid In :", cb, bf, 8, height, row, 100f);
			str = XOutStatusReportBuilder.formatCurrency(credit_card_in);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			XOutStatusReportBuilder.doText("Paid Out :", cb, bf, 8, height, row, 100f);
			str = XOutStatusReportBuilder.formatCurrency(credit_card_out);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			XOutStatusReportBuilder.doText("Net :", cb, bf, 8, height, row, 100f);
			str = XOutStatusReportBuilder.formatCurrency(credit_card_in.subtract(credit_card_out));
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			XOutStatusReportBuilder.doText("Account", cb, bf, 9, height, row, left_margin);
			XOutStatusReportBuilder.doText("Paid In :", cb, bf, 8, height, row, 100f);
			str = XOutStatusReportBuilder.formatCurrency(account_in);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			XOutStatusReportBuilder.doText("Paid Out :", cb, bf, 8, height, row, 100f);
			str = XOutStatusReportBuilder.formatCurrency(account_out);
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			XOutStatusReportBuilder.doText("Net :", cb, bf, 8, height, row, 100f);
			str = XOutStatusReportBuilder.formatCurrency(account_in.subtract(account_out));
			XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;

			if (lab_in.compareTo(BigDecimal.ZERO) != 0 || lab_out.compareTo(BigDecimal.ZERO) != 0)
			{
				XOutStatusReportBuilder.doText("Lab Orders", cb, bf, 9, height, row, left_margin);
				XOutStatusReportBuilder.doText("Paid In :", cb, bf, 8, height, row, 100f);
				str = XOutStatusReportBuilder.formatCurrency(lab_in);
				XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
				XOutStatusReportBuilder.doText("Paid Out :", cb, bf, 8, height, row, 100f);
				str = XOutStatusReportBuilder.formatCurrency(lab_out);
				XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
				XOutStatusReportBuilder.doText("Net :", cb, bf, 8, height, row, 100f);
				str = XOutStatusReportBuilder.formatCurrency(lab_in.subtract(lab_out));
				XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			}
			...
			if (credit_card_tenders.size() > 0)
			{
				XOutStatusReportBuilder.doText("Credit Card Listing - " + credit_card_tenders.size(), cb, bf, 9, height, row, left_margin); row++;
				Iterator itr = credit_card_tenders.iterator();
				while (itr.hasNext())
				{
					TenderRet credit_card_tender = (TenderRet)itr.next();
					UKOnlinePersonBean client = XOutStatusReportBuilder.getClientForTender(credit_card_tender);
					XOutStatusReportBuilder.doText(((credit_card_tender.getOrders().size() > 1) ? "*" : "") + " " + client.getLabel(), cb, bf, 8, height, row, 75f);
					str = XOutStatusReportBuilder.formatCurrency(credit_card_tender.getAmountBigDecimal());
					XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
					listing_total = listing_total.add(credit_card_tender.getAmountBigDecimal());
				}
				XOutStatusReportBuilder.doText("Credit Card Total ", cb, bf, 9, height, row, left_margin);
				str = XOutStatusReportBuilder.formatCurrency(listing_total);
				XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			}
			...
			if (check_tenders.size() > 0)
			{
				XOutStatusReportBuilder.doText("Check Listing - " + check_tenders.size(), cb, bf, 9, height, row, left_margin); row++;
				Iterator itr = check_tenders.iterator();
				while (itr.hasNext())
				{
					TenderRet check_tender = (TenderRet)itr.next();
					UKOnlinePersonBean client = XOutStatusReportBuilder.getClientForTender(check_tender);
					XOutStatusReportBuilder.doText(((check_tender.getOrders().size() > 1) ? "*" : "") + check_tender.getCheckNumber() + " " + client.getLabel(), cb, bf, 8, height, row, 75f);
					str = XOutStatusReportBuilder.formatCurrency(check_tender.getAmountBigDecimal());
					XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
					listing_total = listing_total.add(check_tender.getAmountBigDecimal());
				}
				XOutStatusReportBuilder.doText("Check Total ", cb, bf, 9, height, row, left_margin);
				str = XOutStatusReportBuilder.formatCurrency(listing_total);
				XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			}
			...
			if (cash_tenders.size() > 0)
			{
				XOutStatusReportBuilder.doText("Cash Listing - " + cash_tenders.size(), cb, bf, 9, height, row, left_margin); row++;
				Iterator itr = cash_tenders.iterator();
				while (itr.hasNext())
				{
					TenderRet cash_tender = (TenderRet)itr.next();
					UKOnlinePersonBean client = XOutStatusReportBuilder.getClientForTender(cash_tender);
					XOutStatusReportBuilder.doText(((cash_tender.getOrders().size() > 1) ? "*" : "") + " " + client.getLabel(), cb, bf, 8, height, row, 75f);
					str = XOutStatusReportBuilder.formatCurrency(cash_tender.getChangeAmountBigDecimal());
					XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 155f));
					str = XOutStatusReportBuilder.formatCurrency(cash_tender.getTenderAmountBigDecimal());
					XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
					listing_total = listing_total.add(cash_tender.getTenderAmountBigDecimal().subtract(cash_tender.getChangeAmountBigDecimal()));
				}
				XOutStatusReportBuilder.doText("Cash Total ", cb, bf, 9, height, row, left_margin);
				str = XOutStatusReportBuilder.formatCurrency(listing_total);
				XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			}
			...
			{
				XOutStatusReportBuilder.doText("Account Listing - " + (account_tenders.size() + payment_on_account_tenders.size()), cb, bf, 9, height, row, left_margin); row++;
				Iterator itr = account_tenders.iterator();
				while (itr.hasNext())
				{
					TenderRet account_tender = (TenderRet)itr.next();
					UKOnlinePersonBean client = XOutStatusReportBuilder.getClientForTender(account_tender);
					XOutStatusReportBuilder.doText(((account_tender.getOrders().size() > 1) ? "*" : "") + " " + client.getLabel(), cb, bf, 8, height, row, 75f);
					str = XOutStatusReportBuilder.formatCurrency(account_tender.getAmountBigDecimal());
					XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
				...
				while (itr.hasNext())
				{
					...
					str = XOutStatusReportBuilder.formatCurrency(payment_on_account_tender);
					XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
					listing_total = listing_total.add(payment_on_account_tender);
				}
				
				XOutStatusReportBuilder.doText("Account Total ", cb, bf, 9, height, row, left_margin);
				str = XOutStatusReportBuilder.formatCurrency(listing_total);
				XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			}
			...
			if (gift_card_tenders.size() > 0)
			{
				XOutStatusReportBuilder.doText("Gift Card Listing - " + gift_card_tenders.size(), cb, bf, 9, height, row, left_margin); row++;
				Iterator itr = gift_card_tenders.iterator();
				while (itr.hasNext())
				{
					TenderRet gift_card_tender = (TenderRet)itr.next();
					UKOnlinePersonBean client = XOutStatusReportBuilder.getClientForTender(gift_card_tender);
					XOutStatusReportBuilder.doText(((gift_card_tender.getOrders().size() > 1) ? "*" : "") + " " + client.getLabel(), cb, bf, 8, height, row, 75f);
					str = XOutStatusReportBuilder.formatCurrency(gift_card_tender.getAmountBigDecimal());
					XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
					listing_total = listing_total.add(gift_card_tender.getAmountBigDecimal());
				}
				XOutStatusReportBuilder.doText("Gift Card Total ", cb, bf, 9, height, row, left_margin);
				str = XOutStatusReportBuilder.formatCurrency(listing_total);
				XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			}
			...
			if (gift_certificate_tenders.size() > 0)
			{
				XOutStatusReportBuilder.doText("Gift Certificate Listing - " + gift_certificate_tenders.size(), cb, bf, 9, height, row, left_margin); row++;
				Iterator itr = gift_certificate_tenders.iterator();
				while (itr.hasNext())
				{
					TenderRet gift_certificate_tender = (TenderRet)itr.next();
					UKOnlinePersonBean client = XOutStatusReportBuilder.getClientForTender(gift_certificate_tender);
					XOutStatusReportBuilder.doText(((gift_certificate_tender.getOrders().size() > 1) ? "*" : "") + " " + client.getLabel(), cb, bf, 8, height, row, 75f);
					str = XOutStatusReportBuilder.formatCurrency(gift_certificate_tender.getAmountBigDecimal());
					XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
					listing_total = listing_total.add(gift_certificate_tender.getAmountBigDecimal());
				}
				XOutStatusReportBuilder.doText("Gift Certificate Total ", cb, bf, 9, height, row, left_margin);
				str = XOutStatusReportBuilder.formatCurrency(listing_total);
				XOutStatusReportBuilder.doText(str, cb, bf, 8, height, row, XOutStatusReportBuilder.getXForNumberStr(str, 200f)); row++;
			}
			...
			//int rows_consumed = XOutStatusReportBuilder.doParagraph("You will need to keep your receipts for insurance purposes.", 34, (int)getHeightForRow(height, row), 200, cb, bf, 8, (int)height_per_row, 3);
